...
extern volatile uint32_t SysTickButtonHandlerCounter;
extern LED leds[];
extern volatile uint32_t LedFlags[];
extern const uint32_t LED_QUANTITY;
...
void TIM6_DAC_IRQHandler(void)
{
  /* USER CODE BEGIN TIM6_DAC_IRQn 0 */
	    ...
	    if (SW2Flag==1) //blink on/off
		    { counter++;}

		for (uint32_t n=0; n<LED_QUANTITY; n++)
		  {
		    if (LedFlags[n]==1)
		      {HAL_GPIO_TogglePin(leds[n].port, leds[n].pin);LedFlags[n]=0;}
		    else {}
		  }
